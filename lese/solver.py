...
from dora import get_xp, is_xp, Link
from . import augment, distrib
...
class Solver(object):
    def __init__(self, data, model, optimizer, args):
        ...
        if is_xp():
            ...
            self.link = Link()
        self.history = self.link.history  # Keep track of loss
    ...
    def train(self):
        ...
        test_metrics = {}
        for epoch in range(len(self.link.history), self.epochs):
            # Train one epoch
            self.model.train()
            start = time.time()
            logger.info('-' * 70)
            ...
            best_loss = min(pull_metric(self.link.history, 'valid') + [valid_loss])
            ...
            if (epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1 and self.tt_loader:
                ...
                test_metrics = {'pesq': pesq, 'stoi': stoi}

                # enhance some samples
                logger.info('Enhance and save samples...')
                enhance(self.args, self.model, self.samples_dir)

            self.link.push_metrics({**metrics, **test_metrics})
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                if self.args.lexical_loss:
                    if self.dmodel.module:
                        lexical_loss = F.mse_loss(self.dmodel.module._extract_feat(estimate), self.dmodel.module._extract_feat(clean))
                    else:
                        lexical_loss = F.mse_loss(self.dmodel._extract_feat(estimate), self.dmodel._extract_feat(clean))
                    loss += lexical_loss * self.args.lexical_loss_coef
